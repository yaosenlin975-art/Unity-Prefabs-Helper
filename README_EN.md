...
An independent Unity persistence package for settings and structured save data. It groups values by C# type, reads and writes them through `PrefsHelper`, and provides an Editor tree for inspecting and managing the current project's archives. It does not require the rest of the Lin framework.
...
### Simple example: save one setting

For example, store volume in the `float` archive. You can put this code in `VolumePrefsExample.cs`:
...
Call `Load()` when the game starts and `Save(volume)` when the volume changes. The first read returns the fallback `1f`; that fallback is not saved automatically.

`PrefsHelper` also supports complex structures that Newtonsoft.Json can serialize, including nested objects, lists, and dictionaries. The next example combines them in a multi-slot player save.

### Complex example: nested save data
